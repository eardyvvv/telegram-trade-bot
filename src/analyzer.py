...
impact — Формат: два коротких предложения в ОДНУ строку, разделённых точкой.
  Первое предложение — контекст. Второе — ВСЕГДА начинается с "Возможное влияние на:" и список активов.
  Пример: "Замедление роста может снизить ожидания по ставке. Возможное влияние на: доллар, Treasuries, S&P 500, золото"
  ВАЖНО: НЕ используй переносы строк (\n) внутри impact — всё в одну строку.
